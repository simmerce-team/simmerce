...
export async function getUserConversations(userId: string): Promise<{
  ...
  try {
    ...
    const conversations = uniqueConvs.map(conv => ({
      id: conv.id,
      product_name: Array.isArray(conv.product) ? conv.product[0]?.name : conv.product?.name || 'Product',
      business_name: Array.isArray(conv.business) ? conv.business[0]?.name : conv.business?.name || 'Business',
